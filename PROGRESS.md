...
- **2026-09-13 — v1.0.81** — Replaced the "Shop by Category" circle photos with icons: Solar icon set (`@iconify-json/solar` on npm, MIT licensed), `bold-duotone` style, recolored from Solar's default single-color-plus-50%-opacity duotone to a true two-tone — primary shapes in `--accent-600` (brand blue), secondary/background shapes in `--sale` (brand red) — by swapping `fill="currentColor"` for blue and every `opacity=".5"` for a solid red fill instead. Added `exmart_category_icon_svg( $slug )` in `inc/template-tags.php`, mapping the theme's 8 known top-level category slugs (from `exmart_get_nav_categories()`: personal-care → cosmetic, hair-care → scissors, skin-care → dropper, baby-care → hand-heart, feminine-care → heart, home-care → home, home-diagnostics → stethoscope, health-protection → shield-check) to hand-picked Solar icons, with a generic bag icon as the fallback for any other/future category. Circles now have a soft `--accent-100` tinted background instead of relying on a photo to fill them. Removed `exmart_category_image_url()` (WooCommerce-thumbnail-or-placeholder-photo lookup) since it lost its only caller — its `exmart_category_placeholder_image()` helper is still used elsewhere and was left in place.
