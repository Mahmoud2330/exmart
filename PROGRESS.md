...
4. ~~**Product detail (PDP)**~~ — **Figma match (1.0.75):** qty+ATC+wishlist, reviews summary, related rail  
...
- **2026-09-17 — v1.0.96** — Hero and category carousel fades made denser: those two crop a whole tile/circle rather than a sliver of a card, so the previous thin/subtle gradient still let the cropped item peek through. Widened to `clamp(56px,18vw,110px)` and switched to a gradient that holds solid white until 55% before fading, fully hiding the cropped item instead of just hinting at it. Product rails / brands / trust strip keep the original narrower, subtler fade.
- **2026-09-17 — v1.0.95** — Carousel edge fade scoped to mobile only: the whole `.em-carousel-viewport::after` rule now lives inside `@media (max-width: 767px)`, so desktop never shows it. Also restored the fade on the hero bento carousel (re-added `em-carousel-viewport` to its wrapper in `front-page.php`) — every carousel (hero, product rails, Shop by Category, Our Brands, trust-strip marquee) now shows the right-edge white fade only on small screens.
- **2026-09-17 — v1.0.94** — Carousel edge fade adjusted per feedback: dropped the `backdrop-filter: blur` glass effect (plain white gradient only now), removed the left-side fade entirely (right side only), and removed the fade from the hero bento carousel altogether. `.em-carousel-viewport` in `assets/css/site.css` now renders only a right-edge `::after` gradient; the hero section no longer carries the class.
- **2026-09-17 — v1.0.93** — Added a white glassmorphic edge fade (gradient + `backdrop-filter: blur`, same glass language as `.em-site-header`) to every horizontally-scrolling carousel on Home, so a card cropped by the viewport edge fades to white instead of ending in a hard cut line: hero bento, product rails (Best Sellers/Offers/New Arrivals), Shop by Category strip, Our Brands strip, and the mobile trust-strip marquee. New shared `.em-carousel-viewport` class in `assets/css/site.css` (`::before`/`::after` fade layers), applied via small wrapper/class additions in `front-page.php` and `exmart_product_rail()` (`inc/template-tags.php`, wraps `.em-rail-track` in a new `.em-rail-viewport`). Each carousel's fade auto-disables at the breakpoint where that carousel itself stops scrolling (hero grid at 900px+, category wrap at 640px+, trust strip centers at 768px+) — the always-scrolling rail and brand strips keep the fade at every width.
- **2026-09-17 — v1.0.92** — "Shop by Category" spheres: on mobile (≤639px), tapping a category no longer scales the circle itself (which didn't push neighboring circles out of the way, so it just looked like an overlap glitch) — the scale-on-tap now happens on the image inside instead, clipped by the circle's existing `overflow:hidden`, so the container/border stays fixed size while the artwork zooms in slightly. Desktop hover behavior (circle scales) is unchanged. Added a `@media (max-width: 639px)` block in `assets/css/site.css` covering both `:hover` and `:active` for touch.
- **2026-09-14 — v1.0.91** — Home “Shop by Category”: circle images resolve from Media Library by filename stem (personal-care, hair-care, skin-care, baby-care, home-diagnostics, bundles, home-care-products, wipes) before Unsplash placeholders.
- **2026-09-14 — v1.0.90** — Mobile nav drawer logo reported as "super scaled again". Root cause: `.em-drawer-logo` (height:28px/width:auto/max-width:140px, sized to keep the 1540×844 logo.png small in the drawer) had no `!important`, unlike the near-identical `.em-footer-logo` fix which already needed `!important` to survive the cascade for this exact same file — so `.em-drawer-logo` was the weaker of the two and losing to something else, letting the logo balloon back toward its native size. Hardened `.em-drawer-logo` in `assets/css/site.css` with `!important` on every property to match `.em-footer-logo`'s proven fix.
- **2026-09-14 — v1.0.89** — Added an "outloud" order-confirmation UX: the checkout thank-you page had no clear signal a purchase actually succeeded (still looked/titled like the checkout page). New `woocommerce/checkout/thankyou.php` override restyles it around a big 88px blue (`var(--accent-600)`) circular check icon with a pop-in animation, a bold "Order confirmed!" headline, a clean order-overview card (order number/date/email/total/payment method), and a one-shot CSS confetti burst (skipped under `prefers-reduced-motion`); a red variant covers the failed-payment branch. All core WooCommerce hooks/filters (`woocommerce_thankyou`, `woocommerce_thankyou_{gateway}`, `woocommerce_thankyou_order_received_text`) are preserved so order-details tables, gateway instructions, and tracking plugins keep working. `page.php` now also detects `is_order_received_page()` and swaps the page heading/breadcrumb from "Checkout" to "Order Confirmed" so the page doesn't still read like you're mid-checkout after paying.
- **2026-09-14 — v1.0.88** — Checkout/cart flow on mobile reported as feeling too narrow across the board (fields, shipping/payment cards, buttons, order summary). Code audit found the existing rules already targeted `width: 100%` almost everywhere, so added a defensive `@media (max-width: 767px)` block in `assets/css/site.css` that forces `width/max-width: 100% !important; box-sizing: border-box !important;` on every checkout/cart component (`.em-checkout-layout/-main/-aside/-customer/-step/-fields`, form inputs/selects/textareas, `.select2-container` in case WooCommerce's country/state enhanced-select JS is active, ship/payment cards, place-order/continue buttons, order summary card, pipeline, and the cart equivalents) — `!important` also wins over any stray JS-computed inline width (e.g. select2 initializing against a hidden step), so this closes out any remaining narrow-width edge case regardless of source.
- **2026-09-14 — v1.0.87** — Homepage trust strip on mobile upgraded from a manually-scrollable row to a true auto-scrolling marquee: the item list is duplicated once (duplicate set is `aria-hidden`) and the track animates `translateX(0)` → `translateX(-50%)` on an 18s linear infinite loop for a seamless wrap, pausing on hover and falling back to a static scrollable row when `prefers-reduced-motion: reduce`. Desktop layout (centered, wrapping) is unchanged. Changed `front-page.php` (duplicated `.em-trust-item` markup) and `assets/css/site.css` (`.em-trust-strip-viewport`, `em-trust-marquee` keyframes).
- **2026-09-14 — v1.0.86** — Homepage trust strip (the row of "Authenticated / Cash on delivery / Free returns / Nationwide delivery" items directly below "Our Brands") now scrolls horizontally in a single row on mobile instead of wrapping onto multiple lines. Added `@media (max-width: 767px)` rules on `.em-trust-strip-row`/`.em-trust-item` in `assets/css/site.css` (nowrap + horizontal scroll with snap, hidden scrollbar), matching the pattern already used by the category strip.
- **2026-09-14 — v1.0.85** — Cart COD note redesign per feedback: removed the credit-card icon entirely (cash-only messaging), kept just the cash/banknote icon, and restyled the whole `.em-cart-cod-note` container as a light-green pill using the theme's green tokens (`background: var(--success-bg)`, `color: var(--success)` for icon + text, border matching the light-green background instead of the old neutral grey).
- **2026-09-14 — v1.0.84** — Cart "Cash on delivery available at checkout" note: added a second icon (cash/banknote, matching the existing 24×24 stroke-icon pack style used site-wide) alongside the existing credit-card icon, and recolored the credit-card icon to the theme's green token (`var(--success)`) to visually distinguish card vs. cash. Changed `woocommerce/cart/cart-totals.php` and `assets/css/site.css` (`.em-cod-icons`, `.em-cod-icon-card`).
- **2026-09-13 — v1.0.83** — Fixed Arabic text on cart/checkout buttons (Checkout, View cart, Update cart, Place order, Subtotal, Shipping, Total, etc.). Root cause: recent cart/checkout template overrides used WooCommerce's own text domain (`'woocommerce'`) on literal UI strings instead of the theme's (`'exmart'`), so WordPress applied WooCommerce's bundled Arabic translations to the theme's own markup whenever the site locale is Arabic. Fixed across `woocommerce/cart/cart.php`, `cart-totals.php`, `mini-cart.php`, `proceed-to-checkout-button.php`, and `woocommerce/checkout/form-checkout.php`, `payment.php`, `form-shipping.php`, `review-order.php`, `form-billing.php` — changed the domain argument of every affected `__()/_e()`-family call to `'exmart'` while leaving all `apply_filters('woocommerce_...', ...)` hook/filter names untouched.
