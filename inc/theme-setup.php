...
/**
 * Customizer: pick hero image(s) from the Media Library
 * (replaces the old theme's Swiper slide images).
 *
...
function exmart_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		...
		array(
			...
			'description' => __( 'Comma-separated Media Library attachment IDs for a rotating hero (old Swiper slides). Leave blank for a single image.', 'exmart' ),
			'section'     => 'exmart_hero',
			'type'        => 'text',
		)
	);
